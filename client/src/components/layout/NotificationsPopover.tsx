...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({ isOpen, onClose }) => {
  ...
  const pendingInvitations =
    rsvps?.filter((rsvp) => {
      return (rsvp.status === 'maybe' || rsvp.status === 'pending') && rsvp.userId === user?.id;
    }) || [];
  ...
  const handleJoinRequestResponse = (
    teamId: number,
    requestId: number,
    status: string,
  ...
  const groupInviteResponseMutation = useMutation({
    mutationFn: async ({ groupId, action }: { groupId: number; action: string }) => {
      ...
      const notificationsResponse = await fetch(
        `/api/users/${user?.id}/group-notifications?history=true`,
        {
          credentials: 'include',
        }
      );
      ...
      let invitation = notifications.find(
        (n: any) => n.groupId === groupId && n.type === 'invitation' && n.viewed === false
      );

      if (!invitation) {
        // If no unviewed invitation found, get any invitation for this group
        invitation = notifications.find(
          (n: any) => n.groupId === groupId && n.type === 'invitation'
        );
      }
      ...
      const res = await apiRequest('PUT', `/api/sports-groups/${groupId}/invitation/${requestId}`, {
        action,
      });
      if (!res.ok) {
        const errorData = await res.json();
    ...
    onSuccess: (data, variables) => {
      ...
      toast({
        title: 'Success',
        description:
          variables.action === 'accept' ? "You've joined the group!" : 'Invitation declined',
        variant: 'default',
      });
    },
    onError: (error: Error) => {
  ...
  const respondToTournamentInvitationMutation = useMutation({
    mutationFn: async ({ invitationId, status }: { invitationId: number; status: string }) => {
      const res = await apiRequest('PUT', `/api/tournament-invitations/${invitationId}`, {
        status,
      });
      if (!res.ok) {
        const errorData = await res.json();
  ...
  const handleTournamentInviteResponse = (
    invitationId: number,
    status: string,
    event: React.MouseEvent
  ) => {
      ...
      <div className="max-h-80 overflow-y-auto">
        {totalNotifications > 0 ? (
          <div>
                  ...
                  <div className="flex items-start">
                    <Avatar className="h-10 w-10 mr-3 flex-shrink-0">
                      {invitation.event?.creator?.profileImage ? (
                        <AvatarImage
                          src={invitation.event.creator.profileImage}
                          alt={invitation.event?.creator?.name || 'User'}
                        />
                      ) : (
                        <AvatarFallback>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium">
                        <span className="text-primary">
                          {invitation.event?.creator?.name || 'Someone'}
                        </span>{' '}
                        invited you to{' '}
                        <span className="text-primary">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex mt-2 space-x-2">
                        <Button
                          ...
                          onClick={(e) =>
                            handleJoinRequestResponse(request.teamId, request.id, 'rejected', e)
                          }
                        >
                          <XIcon className="h-3 w-3 mr-1" />
                          Decline
                        ...
                        <Button
                          ...
                          disabled={respondToJoinRequestMutation.isPending}
                          onClick={(e) =>
                            handleJoinRequestResponse(request.teamId, request.id, 'accepted', e)
                          }
                        >
                          <CheckIcon className="h-3 w-3 mr-1" />
                          Accept
            ...
            {typedFriendRequests &&
              typedFriendRequests.map((request: any) => (
            ...
            {teamMemberNotifications &&
              teamMemberNotifications.map((notification, index) => (
                <div
                ...
                >
                  <div className="flex items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium">Your request to join has been accepted</p>
                      <p className="text-sm font-semibold text-primary">
                ...
                if (notification.type === 'invitation') {
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex mt-2 space-x-2">
                            <Button
                              ...
                              onClick={(e) =>
                                handleGroupInviteResponse(notification.groupId, 'decline', e)
                              }
                            >
                              <XIcon className="h-3 w-3 mr-1" />
                              Decline
                            ...
                            <Button
                              ...
                              disabled={groupInviteResponseMutation.isPending}
                              onClick={(e) =>
                                handleGroupInviteResponse(notification.groupId, 'accept', e)
                              }
                            >
                              <CheckIcon className="h-3 w-3 mr-1" />
                              Accept
                  ...
                  return (
                    <Link
                      ...
                      onClick={() => {
                        markNotificationsViewed.mutate({
                              ...
                              {notification.type === 'message'
                                ...
                                : notification.type === 'poll'
                                  ...
                                  : notification.type === 'event'
                                    ? 'new events'
                                    : 'new activities'}{' '}
                              in <span className="text-primary">{notification.groupName}</span>
                            </p>
                            <p className="text-xs text-gray-500 mt-1">
            ...
            {tournamentInvitations &&
              tournamentInvitations.map((invitation) => (
                <div key={`tournament-${invitation.id}`} className="p-3 hover:bg-gray-50 border-b">
                  <Link href={`/tournaments/${invitation.tournamentId}`} onClick={onClose}>
                    <div className="flex items-start cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium">
                          <span className="text-primary">{invitation.inviterName}</span> invited you
                          to{' '}
                        ...
                        <div className="mt-1 space-y-1">
                          ...
                          {invitation.startDate && (
                            <p className="text-xs text-gray-600">
                              Starts:{' '}
                              {new Date(invitation.startDate).toLocaleDateString('en-US', {
                                month: 'short',
                                day: 'numeric',
                                year: 'numeric',
                                hour: '2-digit',
      ...
      <div className="p-2 border-t">
        <div className="flex justify-center">
          <Link href="/notifications">
            <Button variant="ghost" className="text-primary text-sm" size="sm" onClick={onClose}>
              View History
            </Button>
          </Link>
        </div>
