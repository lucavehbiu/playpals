...
export function useWebSocket() {
          ...
          (newNotif) =>
            !prev.some(
              (existingNotif) =>
                ...
                (newNotif.requestId &&
                  existingNotif.requestId === newNotif.requestId &&
  ...
  const pollNotifications = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiRequest(
        'GET',
        `/api/users/${user.id}/notifications?since=${lastFetchTime.current}`
      );
  ...
  const startPolling = useCallback(() => {
    ...
    setStatus('fallback');
    console.log(
      'WebSocket disconnected. Notifications temporarily disabled to prevent browser crashes.'
    );
    ...
    toast({
      ...
      description:
        'Notification system is temporarily unavailable. Please refresh the page in a few moments.',
      variant: 'default',
      duration: 5000,
    });
  ...
  const connect = useCallback(() => {
    ...
    if (reconnectAttempts.current >= MAX_RECONNECT_ATTEMPTS) {
      console.log(
        `Max reconnection attempts (${MAX_RECONNECT_ATTEMPTS}) reached. Switching to polling fallback.`
      );
      startPolling();
      return;
    }
    ...
    try {
      ...
      console.log(
        `Connecting to WebSocket server at: ${wsUrl} (attempt ${reconnectAttempts.current + 1}/${MAX_RECONNECT_ATTEMPTS})`
      );
      ...
      newSocket.onmessage = (event) => {
        try {
          ...
          if (data.type === 'auth_success') {
            ...
            data.type === 'join_request_update' ||
            data.type === 'rsvp_approved'
          ) {
      ...
      newSocket.onerror = (error) => {
        ...
        console.log(
          `Will attempt to reconnect in ${delay}ms (attempt ${reconnectAttempts.current}/${MAX_RECONNECT_ATTEMPTS})`
        );
      ...
      newSocket.onclose = (event) => {
        ...
        if (user && user.id && event.code !== 1000 && event.code !== 1001) {
          ...
          if (reconnectAttempts.current < MAX_RECONNECT_ATTEMPTS) {
            ...
            const delay = INITIAL_RECONNECT_DELAY * Math.pow(2, reconnectAttempts.current - 1);
            console.log(
              `Connection closed. Will attempt to reconnect in ${delay}ms (attempt ${reconnectAttempts.current}/${MAX_RECONNECT_ATTEMPTS})`
            );
