...
async function checkAndNotifyNewSuggestions(
  pollId: number,
  groupId: number,
  ...
  try {
    console.log(
      `Checking for new suggestions after user ${responseUserId} responded to poll ${pollId}`
    );
    ...
    for (const slot of timeSlots) {
      ...
      console.log(
        `Slot ${slot.id} (${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][slot.dayOfWeek]} ${slot.startTime}-${slot.endTime}): ${availableCount} available, min required: ${poll.minMembers || 2}, used for event: ${slot.usedForEventId}`
      );
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post(
    '/api/teams/:teamId/posts/:postId/image',
    authenticateUser,
    upload.single('image'),
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/tournaments/:tournamentId/image',
    authenticateUser,
    upload.single('image'),
    async (req: Request, res: Response) => {
      try {
  ...
  app.get('/api/users/all', async (req: Request, res: Response) => {
    try {
      ...
      const sanitizedUsers = users.map(
        ({ password, ...userData }: { password: string; [key: string]: any }) => userData
      );
  ...
  app.get('/api/events/:id([0-9]+)', async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        'Found event with creator info:',
        event.title,
        'Creator:',
        event.creator?.name || 'Not found'
      );
  ...
  app.put('/api/events/:id', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (event.creatorId !== authenticatedUser.id) {
        return res
          .status(403)
          .json({ message: 'Forbidden - You can only update events you created' });
      }
  ...
  app.delete('/api/events/:id', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (event.creatorId !== authenticatedUser.id) {
        return res
          .status(403)
          .json({ message: 'Forbidden - You can only delete events you created' });
      }
  ...
  app.put('/api/rsvps/:id', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        'DEBUG: RSVP update - req.body.status:',
        req.body.status,
        'rsvp.status:',
        rsvp.status
      );
        ...
        try {
          ...
          if (event) {
            ...
            if (groupEvent) {
              ...
              const approvedParticipants = allRSVPs.filter(
                (r) => r.status === 'approved' && r.userId !== authenticatedUser.id
              );
              ...
              for (const participantRSVP of approvedParticipants) {
                ...
                try {
                  ...
                  console.log(
                    'DEBUG: Successfully created leave notification for user:',
                    participantRSVP.userId
                  );
                } catch (notificationError) {
                  console.error(
                    'Error creating leave notification for user',
                    participantRSVP.userId,
                    ':',
                    notificationError
                  );
  ...
  app.get(
    '/api/onboarding-preferences/:userId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.post(
    '/api/onboarding-preferences/:userId/complete',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userIdParam = parseInt(req.params.userId);
  ...
  app.get('/api/teams', async (req: Request, res: Response) => {
    try {
      ...
      const teamsWithCreatorInfo = await Promise.all(
  ...
  app.put('/api/teams/:id', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (team.creatorId !== authenticatedUser.id && (!member || member.role !== 'admin')) {
        return res
          .status(403)
          .json({ message: "Forbidden - You don't have permission to update this team" });
      }
  ...
  app.delete('/api/teams/:id', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (team.creatorId !== authenticatedUser.id) {
        return res
          .status(403)
          .json({ message: 'Forbidden - Only the team creator can delete the team' });
      }
  ...
  app.post('/api/teams/:teamId/members', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        (!adminMember || adminMember.role !== 'admin')
      ) {
        return res
          .status(403)
          .json({ message: "Forbidden - You don't have permission to add members to this team" });
      }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const isAdmin =
          team.creatorId === authenticatedUser.id || (authMember && authMember.role === 'admin');
        ...
        if (!isAdmin && !isCaptain && !isSelf) {
          return res
            .status(403)
            .json({ message: "Forbidden - You don't have permission to update this team member" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!isAdmin && !isSelf) {
          return res
            .status(403)
            .json({ message: "Forbidden - You don't have permission to remove this team member" });
        }
  ...
  app.post('/api/teams/:teamId/posts', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (!member) {
        return res
          .status(403)
          .json({ message: 'Forbidden - You must be a team member to create posts' });
      }
      ...
      if (
        ...
        return res
          .status(403)
          .json({ message: "Forbidden - You don't have permission to create posts in this team" });
      }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!isCreator && !isTeamAdmin && !isTeamMemberAdmin) {
          return res
            .status(403)
            .json({ message: "Forbidden - You don't have permission to delete this post" });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!member) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You must be a team member to comment on posts' });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!member) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You must be a team member to create schedules' });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!member) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You must be a team member to respond to schedules' });
        }

        // Check if response already exists
        const existingResponse = await storage.getTeamScheduleResponse(
          scheduleId,
        ...
        try {
          ...
          return res
            .status(400)
            .json({ message: 'Invalid response data', details: validationError });
        }
      } catch (error) {
        if (error instanceof z.ZodError) {
  ...
  app.put(
    '/api/schedules/:scheduleId/responses/:responseId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const scheduleId = parseInt(req.params.scheduleId);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (existingRequest) {
          return res
            .status(400)
            .json({ message: 'You already have a pending join request for this team' });
        }
  ...
  app.get(
    '/api/teams/:teamId/join-request-status',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const teamId = parseInt(req.params.teamId);
        ...
        if (authenticatedUser.id !== userId) {
          return res
            .status(403)
            .json({ message: "Not authorized to check this user's join request" });
        }
      ...
      try {
        ...
        if (team.creatorId !== authenticatedUser.id) {
          ...
          if (!member || member.role !== 'admin') {
            return res
              .status(403)
              .json({ message: 'Forbidden - Only team admins can view join requests' });
          }
        }
      ...
      try {
        ...
        if (team.creatorId !== authenticatedUser.id) {
          ...
          if (!member || member.role !== 'admin') {
            return res
              .status(403)
              .json({ message: 'Forbidden - Only team admins can process join requests' });
          }
        }
  ...
  app.get(
    '/api/teams/join-notifications/:userId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only access your own notifications' });
        }
  ...
  app.post(
    '/api/teams/join-notifications/:notificationId/viewed',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        ...
        if (notification.userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: "Forbidden - This notification doesn't belong to you" });
        }
  ...
  app.get(
    '/api/users/:userId/team-join-history',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only access your own team join history' });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only access your own RSVP history' });
        }
    ...
    if (action === 'my') {
      ...
      try {
        ...
        const discoverableGroups = allGroups.filter(
          (group) => !userGroupIds.includes(group.id) && !group.isPrivate
        );
  ...
  app.get(
    '/api/sports-groups/:id/events/history',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.id);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!member) {
          return res
            .status(403)
            .json({ message: 'You must be a member to add events to this group' });
        }
  ...
  app.get(
    '/api/users/:userId/group-notifications',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.get(
    '/api/users/:userId/unread-events/:groupId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.get(
    '/api/users/:userId/unread-messages/:groupId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.post(
    '/api/users/:userId/group-notifications/view',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.post(
    '/api/sports-groups/:groupId/join-request',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
        ...
        const existingRequest = await storage.getSportsGroupJoinRequest(
          groupId,
          authenticatedUser.id
        );
        if (existingRequest) {
  ...
  app.post(
    '/api/sports-groups/:groupId/invite',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
  ...
  app.put(
    '/api/sports-groups/:groupId/invitation/:requestId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Friend requests check: authenticated user ${authenticatedUser.id} accessing user ${userId} friend requests`
        );
        if (authenticatedUser.id !== userId) {
          console.log(
            `Friend requests access denied: authenticated user ${authenticatedUser.id} tried to access user ${userId} friend requests`
          );
          return res.status(403).json({ message: 'Access denied' });
        }
  ...
  app.get(
    '/api/users/:userId/all-friend-requests',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Friend request acceptance check: friendRequest.friendId=${friendRequest.friendId}, authenticatedUser.id=${authenticatedUser.id}`
        );
        if (friendRequest.friendId !== authenticatedUser.id) {
          console.log(
            `Access denied: Only recipient (${friendRequest.friendId}) can accept, but user ${authenticatedUser.id} tried`
          );
          return res.status(403).json({ message: 'Access denied' });
        }
  ...
  app.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          friendship.userId !== authenticatedUser.id &&
  ...
  app.get(
    '/api/sports-groups/:groupId/polls',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
  ...
  app.get(
    '/api/sports-groups/:groupId/polls/:pollId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
  ...
  app.delete('/api/sports-groups/:groupId/polls/:pollId', async (req: Request, res: Response) => {
    try {
      ...
      if (!isCreator && !isAdmin) {
        return res
          .status(403)
          .json({ message: 'Only the poll creator or group admin can delete this poll' });
      }
  ...
  app.post(
    '/api/sports-groups/:groupId/polls/:pollId/responses',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const pollId = parseInt(req.params.pollId);
        ...
        if (!membership) {
          return res
            .status(403)
            .json({ message: 'You must be a group member to respond to polls' });
        }

        const { availability } = req.body; // Custom availability object

        // Delete existing responses for this user and poll
        const existingResponses = await storage.getSportsGroupPollUserResponses(
          pollId,
          authenticatedUser.id
        );
  ...
  app.get(
    '/api/sports-groups/:groupId/polls/:pollId/user-responses',
    async (req: Request, res: Response) => {
      try {
        // Authentication check with bypass
        const authResult = await checkAuthWithBypass(req, res);
        ...
        console.log(
          `🔍 API CALLED: Fetching user responses for poll ${pollId}, user ${authenticatedUser.id} (${authenticatedUser.name}) - TIME: ${new Date().toISOString()}`
        );
        ...
        const responses = await storage.getSportsGroupPollUserResponses(
          pollId,
          authenticatedUser.id
        );
        console.log(
          `Found ${responses.length} responses for user ${authenticatedUser.id} (${authenticatedUser.name}):`,
          responses
        );
  ...
  app.post(
    '/api/sports-groups/:groupId/polls/:pollId/invite-available-users',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
        ...
        const availableUsers = responses.filter(
          (response) => response.timeSlotId === timeSlotId && response.isAvailable === true
        );
  ...
  app.get(
    '/api/sports-groups/:groupId/polls/:pollId/analysis',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const pollId = parseInt(req.params.pollId);
        ...
        const analysis = timeSlots.map((slot) => {
          ...
          console.log(
            `Slot ${slot.id} (${slot.dayOfWeek}/${slot.startTime}-${slot.endTime}): ${availableCount} available, min needed: ${poll.minMembers}, meets minimum: ${meetsMinimum}`
          );
          ...
          return {
            ...
            availableUserIds: slotResponses
        ...
        const groupedByDay = analysis.reduce(
          (acc, slot) => {
          ...
          daySlots.forEach((slot1) => {
            daySlots.forEach((slot2) => {
              ...
              if (overlapStart < overlapEnd) {
                ...
                const combinedUsers = [
                  ...new Set([...slot1.availableUserIds, ...slot2.availableUserIds]),
                ];
        ...
        console.log(
          `Found ${overlappingOpportunities.length} overlapping opportunities:`,
          overlappingOpportunities
        );
        ...
        console.log(
          `Poll analysis complete. Total slots: ${timeSlots.length}, Direct viable slots: ${sortedSlots.filter((s) => s.meetsMinimum).length}, Overlap opportunities: ${overlappingOpportunities.length}, Total suggestions: ${suggestions.length}`
        );
        ...
        if (suggestions.length > 0) {
          ...
          const allAvailableUserIds = [
            ...new Set(responses.filter((r) => r.isAvailable === true).map((r) => r.userId)),
          ];
          ...
          for (const userId of allAvailableUserIds) {
            try {
              ...
              const existingNotification = await storage
                .getSportsGroupNotifications(userId)
                .then((notifications) =>
                  notifications.find(
                    (n) =>
  ...
  storage.createTeamJoinRequest = async (data) => {
    ...
    if (joinRequest) {
      ...
      if (team) {
        ...
        const adminMembers = teamMembers.filter(
          (member) => member.role === 'admin' && member.userId !== team.creatorId
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          ...
          !['all', 'friends', 'friends_participants'].includes(publicVisibility)
        ) {
        ...
        if (event.creatorId !== authenticatedUser.id) {
          ...
          if (groupEvent) {
            const membership = await storage.getSportsGroupMember(
              groupEvent.groupId,
              authenticatedUser.id
            );
  ...
  app.get(
    '/api/events/:eventId/match-result',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const eventId = parseInt(req.params.eventId);
  ...
  app.get(
    '/api/groups/:groupId/match-results',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!parsed.success) {
          ...
          return res
            .status(400)
            .json({ error: 'Invalid match result data', details: parsed.error.issues });
        }
        ...
        if (!rsvp || rsvp.status !== 'approved') {
          return res
            .status(403)
            .json({ error: 'Only event participants can submit match results' });
        }
        ...
        const participants = [
          ...teamAUsers.map((userId) => ({
            matchId: matchResult.id,
            userId,
            team: 'A' as const,
            isWinner: parsed.data.winningSide === 'A',
          })),
          ...teamBUsers.map((userId) => ({
            matchId: matchResult.id,
            userId,
            team: 'B' as const,
            isWinner: parsed.data.winningSide === 'B',
          })),
        ];
        ...
        try {
          ...
          console.log(
            `Creating score notifications for ${approvedParticipants.length} participants`
          );
          ...
          for (const rsvp of approvedParticipants) {
            if (rsvp.userId !== userId) {
              // Don't notify the person who submitted the score
              if (groupEvent) {
  ...
  app.get(
    '/api/groups/:groupId/player-statistics',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const groupId = parseInt(req.params.groupId);
        ...
        const enhancedStats = await Promise.all(
          statistics.map(async (stat) => {
            ...
            const winRate =
              stat.matchesPlayed > 0 ? (stat.matchesWon / stat.matchesPlayed) * 100 : 0;
  ...
  app.get(
    '/api/users/:userId/match-result-notifications',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.patch(
    '/api/match-result-notifications/:notificationId/viewed',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const notificationId = parseInt(req.params.notificationId);
  ...
  app.put('/api/events/:eventId/score', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        console.log(
          `Sending score edit notifications to ${approvedParticipants.length} participants`
        );
        ...
        console.log(
          `Score edit notifications sent to ${approvedParticipants.length - 1} participants (excluding editor)`
        );
      } catch (error) {
  ...
  app.get(
    '/api/events/:eventId/score-history',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const eventId = parseInt(req.params.eventId);
  ...
  app.get(
    '/api/users/:userId/professional-team-history',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only access your own team history' });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only manage your own team history' });
        }
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: 'Invalid team history data', errors: error.errors });
        }
  ...
  app.put(
    '/api/professional-team-history/:id',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        ...
        if (history.userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only update your own team history' });
        }

        const validatedData = insertProfessionalTeamHistorySchema.partial().parse(req.body);
        const updatedHistory = await storage.updateProfessionalTeamHistory(
          historyId,
          validatedData
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (history.userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only delete your own team history' });
        }
  ...
  app.get(
    '/api/users/:userId/sport-skill-levels',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only manage your own skill levels' });
        }
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: 'Invalid skill level data', errors: error.errors });
        }
  ...
  app.put('/api/sport-skill-levels/:id', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      if (skillLevel.userId !== authenticatedUser.id) {
        return res
          .status(403)
          .json({ message: 'Forbidden - You can only update your own skill levels' });
      }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (skillLevel.userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only delete your own skill levels' });
        }
  ...
  app.put(
    '/api/users/:userId/profile-completion',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only update your own profile completion' });
        }
        ...
        if (typeof completionLevel !== 'number' || completionLevel < 0 || completionLevel > 100) {
          return res
            .status(400)
            .json({ message: 'Completion level must be a number between 0 and 100' });
        }
  ...
  app.put(
    '/api/users/:userId/phone-verification',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        ...
        if (isNaN(userId) || userId !== authenticatedUser.id) {
          return res
            .status(403)
            .json({ message: 'Forbidden - You can only update your own phone verification' });
        }
  ...
  app.get(
    '/api/tournaments/creator/:creatorId',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post('/api/tournaments/:id/invite', authenticateUser, async (req: Request, res: Response) => {
    try {
      ...
      console.log('Tournament invitation request:', {
        tournamentId,
        userIds,
      ...
      for (const userId of userIds) {
        ...
        const existingInvitation = await storage.getTournamentInvitationByTournamentAndUser(
          tournamentId,
          userId
        );
  ...
  app.get(
    '/api/users/:userId/tournament-invitations',
    authenticateUser,
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log('Tournament invitation response:', {
          invitationId,
          status,
          userId: authenticatedUser.id,
        });
        ...
        if (status === 'accepted') {
          ...
          console.log('User joined tournament via invitation:', {
            userId: authenticatedUser.id,
            tournamentId: invitation.tournamentId,
          });

          // Check if tournament is now full and start it automatically
          const updatedParticipants = await storage.getTournamentParticipants(
      ...
      try {
        ...
        if (winnerId) {
          ...
          if (winnerParticipant) {
            ...
            if (currentWinnerStanding) {
              await storage.updateTournamentStanding(currentWinnerStanding.id, {
                ...
                goalDifference:
                  (currentWinnerStanding.goalDifference || 0) +
                  Math.abs((participant1Score || 0) - (participant2Score || 0)),
              });
            }
          }
          ...
          if (loserParticipant) {
            ...
            const currentLoserStanding = loserStanding.find(
              (s) => s.participantId === loserParticipant.id
            );
            ...
            if (currentLoserStanding) {
              await storage.updateTournamentStanding(currentLoserStanding.id, {
                ...
                goalDifference:
                  (currentLoserStanding.goalDifference || 0) -
                  Math.abs((participant1Score || 0) - (participant2Score || 0)),
              });
            }
          }
